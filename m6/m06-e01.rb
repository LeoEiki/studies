...
class MySQLExercises1
  ...
  def q_users_names_and_visited_at
    ...
    <<-SQL
      ...
      FROM users
      WHERE visited_at >= '2016-01-01 00:00:00'
  ...
  def q_products_discounts
    ...
    <<-SQL
      SELECT id, 
      original_price, 
      (1-(price/original_price)) AS discount 
      FROM products;
    SQL
  end
  ...
  def q_sales_count_for_product
    ...
    <<-SQL
      SELECT products.title as title, count(products.id)
      FROM products JOIN sales_products ON sales_products.sale_id = products.id 
  ...
  def q_products_by_popularity
    ...
    <<-SQL
      ...
      products.title, 
      COUNT(sales_products.id) AS num_sales 
      FROM products JOIN sales_products ON sales_products.id = products.id 
      GROUP BY products.id 
      ORDER BY COUNT(products.id) DESC, products.id ASC;
    SQL
  end
  ...
  def q_products_without_category
    ...
    <<-SQL
      SELECT COUNT(category_id) as uncategorized
      FROM products 
