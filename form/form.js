...
function validatePhone(inputName, input) {
  var isValid = isEmpty(input.value) || /^\d{2}\d{8,9}$/.test(input.value)
  return showOrHideErrorMessage(inputName, isValid, 'invalid')
...
function cleanUp(event) {
  var confirm = window.confirm("Tem certeza que deseja apagar o formulário?");
  if (!confirm){
    event.preventDefault();
  }else{
...
function change(event){
  for (i=0;i<elem.length;i++){
    if (elem[i].checked){
          elem[i].parentElement.classList.add("selected");
    }else{
          elem[i].parentElement.classList.remove("selected");
    }
  }
}
...
elem[i].addEventListener("click", change);
}
